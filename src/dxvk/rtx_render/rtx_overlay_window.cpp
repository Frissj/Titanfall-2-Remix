...
LRESULT GameOverlay::overlayWndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
  ...
  case WM_INPUT:
  {
    ...
    // Keyboard raw input: skip.  Keyboard is now handled reliably
    // via the game's WndProc hook (WM_KEYDOWN/WM_SYSKEYDOWN), which
    // works regardless of overlay window state.  Processing keyboard
    // here too could cause double-delivery edge cases.
  ...
  // Let ImGui Win32 backend handle remaining window messages (non-keyboard).
