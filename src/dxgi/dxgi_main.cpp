...
namespace dxvk {
  ...
  static HRESULT forwardToSystemDxgi(UINT Flags, REFIID riid, void** ppFactory) {
    ...
    auto fn = reinterpret_cast<PFN_CreateDXGIFactory>(GetProcAddress(hSys, "CreateDXGIFactory1"));
    if (!fn) return E_FAIL;
  ...
  HRESULT createDxgiFactory(UINT Flags, REFIID riid, void **ppFactory) {
    ...
    if (s_creating)
      return forwardToSystemDxgi(Flags, riid, ppFactory);
