...
| `[Perf.LoopCut]` | `d3d11_rtx.cpp` | **the cut of `loop_ms`, per CALL**: `t31_gather` / `t31_copy` / `inst_loop`, and `sum_us` which must equal `[Perf.InstDraw] loop_ms / calls`. Self-sufficient within one window - read this instead of diffing `loop_ms` across captures. `t31_copy` is the largest single item on the frame thread |
| `[Perf.T31Src]` | `d3d11_rtx.cpp` | mapping lifetime of the t31 source: `sameBufPct` / `samePtrPct` / `sameGenPct` / `pagesPerCall` / `usPerPage` / `usPerKB`. `samePtrPct` ~6% is why `[Perf.T31Cache] hitPct` is 0.0 and cannot be fixed by a better key. **`usPerPage` vs `usPerKB` is NOT a discriminator** - `pagesPerCall` is pinned at 2.20, so they are one measurement twice |
| `[Perf.T31Warm]` | `d3d11_rtx.cpp` | **the double-read probe**: same source range copied 3x, 1 sample in 64. `c1` = first touch (the real copy), `c2` = second touch, `c3` = control on a second destination. **Result: both first-touch and uncached-read were refuted** - min is 0.5-0.7 us for all three in every window, max is 310-5403 us, and `c2` mean sits level with `c1`'s. `t31_copy` is a ~0.7 us body plus a rare tail; the "6.5 us/page" figure was a mean over a bimodal distribution. Excluded from `t31_copy` by design |
| `[Perf.T31Bin]` | `d3d11_rtx.cpp` | `[Perf.T31Warm]` split on `t31SamePtr`. **Result: `renamed c1min` = 0.70 us every window (n=715-786)** - the fresh-slice read runs at cached WB speed, so the CPU read is not the cost and **HANDOFF s5 is worth ~0.4-0.7 ms/frame, not 7.4**. `samePtr`'s higher floor (1.1-1.3) is a size effect: those calls widen the copy to the union of the cached fill and the draw's need |
| `[Perf.T31Hist]` | `d3d11_rtx.cpp` | `c1`/`c2` duration histograms, 6 buckets, raw counts. **Result: the body warms (`<1us` 19 -> 155), the tail does not** - `>=20us` buckets are equal in both rows. ~9% of copies carry ~64% of `t31_copy`, and re-reading identical bytes pays identical cost. The tail is not a property of the read |
| `[Perf.T31Tail]` | `d3d11_rtx.cpp` | **shape of the ~4.7 ms/frame that is not the read.** Timed on EVERY copy, not the 1-in-64 sample, so `gap[]` is in real copies. Geometric `gap[]` decay = ambient interference (search moves off this thread); a `gap[1]` spike = bursty, one event stalling a stretch; `perFrame min==max` = periodic. `tailPct` vs `tailTimePct` is the bimodality restated |
| `[Perf.T31TailPos]` | `d3d11_rtx.cpp` | tail events by position in the frame's copy stream, in eighths. Flat = position-independent; a concentrated eighth names a phase, and its ordinal is comparable to `[SubmitStall]`'s frame ordinal |
| `[Perf.T31TailEvt]` | `d3d11_rtx.cpp` | raw tail events, 1 in 512 (~9/window): frame, ordinal, duration, gap, size. **This is the line that broke the case open** - 52.0 KB->38.5 us, 34.3 KB->25.3 us, 49.8 KB->37.1 us, one rate at 1.38 GB/s. Size-proportional, so the cost is bytes x rate |
| `[Perf.T31Size]` | `d3d11_rtx.cpp` | us and GB/s per size bucket. **Flat `GBs` across buckets = no per-call component left, the only lever is fewer bytes.** A bucket below the line is the genuine-outlier population separated from the size effect |
| `[Perf.T31Span]` | `d3d11_rtx.cpp` | `usedPct` = (distinct entries x 208) / span copied - the waste `[T31Range]`'s union span leaves. **`runsPerDraw` picks the fix**: small = per-run copies collect it, large = per-call overhead eats it and the answer is HANDOFF s5. `ovf` must read 0 or distinct/runs are undercounts |
| `[Perf.FanoutRej]` | `d3d11_rtx.cpp` | share of decoded instances the CPU compaction drops (`oob` / `nonFinite` / `zeroRow0`). **The go/no-go for HANDOFF s5's surface-slot reservation**: `dropPct` 0 means worst-case reservation equals today's count and `[FindStage]` cannot move; non-zero means re-check `surfaceIndexOfFirstInstance` in `rtx_accel_manager` first. Same counters `[SpawnGeomDiag.hist]` prints, but not gated behind `kDiagLogs` |
| `[Perf.FanoutGate]` | `d3d11_rtx.cpp` | census of the `!m_instBufCache.empty() && t31Data` gate: `instPass` / `instMiss` / `missPct`, split by `noT31` / `noCache`, plus an 8-way VS census of the misses. **Explains why `[Perf.FanoutRej] seen` is ~65% of `[Perf.InstDraw] instances`** - different populations, not lost instances. `topVs` all particle/decal/UI closes it; a world or model VS there is instanced geometry skipping the RT fanout and wants its own investigation. `vsKey` is `GetCurrentVsPsHashes`, NOT `getHash()` |
